...
class Detectiveapp(QWidget):
    ...
    def initUI(self):
        ...
        user_input_group = QGroupBox("Search for Usernames (you can search for multiple by separating them with commas[','])")
        ...
        self.username_input.setPlaceholderText(
            "Enter the usernames separated by commas ( username1, username2)"
        )
        ...
        for username in usernames:
            ...
            if username:
                self.result_area.append(
                    f"'{username}' user is being searched...\n"
                )
    ...
    async def search_api(self,username,site,session):
        try:
            ...
            return f"Error with {site['name']} : {str(e)}"
        ...
        if filename:
            if filename.endswith('.txt'):
                with open(filename,'w',encoding='utf-8') as file:
                    ...
                    QMessageBox.information(self,"info","Completed")
if __name__ =='__main__':
    app = QApplication(sys.argv)
    dedektif = Detectiveapp()
    dedektif.show()
    sys.exit(app.exec_())